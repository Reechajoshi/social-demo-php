...
function update_stats($type) {
	...
	$database->database_query("INSERT INTO se_stats (stat_date, stat_$type) VALUES (UNIX_TIMESTAMP(CURDATE()), 1) 
...
function update_refurls() {
	...
	if($referring_url != "") {
	  ...
	  $database->database_query("INSERT INTO se_statrefs (statref_hits, statref_url) VALUES ('1', '$referring_url')
	  ...
	  if($refurl_totalrows >= 1000) { $database->database_query("DELETE FROM se_statrefs WHERE statref_hits='1' ORDER BY statref_id ASC LIMIT 1"); }
	}
