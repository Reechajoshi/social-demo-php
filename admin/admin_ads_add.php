...
while($row = $database->database_fetch_assoc($qShowStatusResult)) {
  $next_increment = $row['Auto_increment'];
}
$ad_id = $next_increment;
...
if($task == "dosave") {
  ...
  if($ad_date_end_options == 0) {
    ...
    if($is_error == 0 AND ($ad_date_end_month == "" OR $ad_date_end_day == "" OR $ad_date_end_year == "" OR $ad_date_end_hour == "" OR $ad_date_end_minute == "" OR $ad_date_end_ampm == "")) {
      $is_error = 1;
      $error_message = $admin_ads_add[4];
    }
    if($is_error == 0 AND $ad_date_end <= $ad_date_start) {
      $is_error = 1;
      $error_message = $admin_ads_add[5];
    }
  }
...
while($level_info = $database->database_fetch_assoc($levels)) {
  ...
  if(in_array($level_info[level_id], $ad_levels_array) OR $task == "main") { 
    $level_selected = 1; 
  } else { 
    $level_selected = 0; 
  }
...
if(in_array("0", $ad_subnets_array) OR $task == "main") {
  $subnet_selected = 1;
} else {
  $subnet_selected = 0;
}
$subnet_array[$subnet_count] = Array('subnet_id' => "0",
				     'subnet_name' => $admin_ads_add[66],
...
while($subnet_info = $database->database_fetch_assoc($subnets)) {
  ...
  if(in_array($subnet_info[subnet_id], $ad_subnets_array) OR $task == "main") { 
    $subnet_selected = 1;
  } else { 
    $subnet_selected = 0; 
  }
...
$smarty->assign("ad_id_next", $next_increment);
