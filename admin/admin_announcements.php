...
if($task == "post" AND $type == "email") {
  ...
  $levels_query = $database->database_query("SELECT level_id, level_name, level_default FROM se_levels");
  $levels = Array();
  $level_count = 0;
  ...
  $subnets_query = $database->database_query("SELECT subnet_id, subnet_name FROM se_subnets");
  $subnets = Array();
  $subnet_count = 0;
