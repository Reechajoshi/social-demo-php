...
$subnets = $database->database_query("SELECT * FROM se_subnets WHERE subnet_id='$subnet_id'");
if($database->database_num_rows($subnets) != 1) {
  header("Location: admin_subnetworks.php?s=$s");
  exit();
}
...
$primary = $database->database_fetch_assoc($database->database_query("SELECT field_title, field_type, field_options, field_birthday FROM se_fields WHERE field_id='$setting[setting_subnet_field1_id]'"));
$secondary = $database->database_fetch_assoc($database->database_query("SELECT field_title, field_type, field_options, field_birthday FROM se_fields WHERE field_id='$setting[setting_subnet_field2_id]'"));

// IF EMAIL DOMAIN SELECTED
if($setting[setting_subnet_field1_id] == "0") {
  $primary['field_title'] = $admin_subnetworks_edit[17];
  $primary[field_type] = 1;
}
if($setting[setting_subnet_field2_id] == "0") {
  $secondary['field_title'] = $admin_subnetworks_edit[17];
  $secondary[field_type] = 1;
}

// CHECK FOR AGE
if($primary[field_birthday] == 1) {
  $primary[field_title] .= " $admin_subnetworks_edit[18]";
  $primary[field_type] = 1;
}
if($secondary[field_birthday] == 1) {
  $secondary[field_title] .= " $admin_subnetworks_edit[18]";
  $secondary[field_type] = 1;
}
...
if($task == "doedit") {
  ...
  if(str_replace(" ", "", $subnet_name) == "") {
    $is_error = 1;
    $error_message = $admin_subnetworks_edit[15];
    ...
    $is_error = 1;
    $error_message = $admin_subnetworks_edit[14];
    ...
    $is_error = 1;
    $error_message = $admin_subnetworks_edit[16];

  // NO ERROR, UPDATE SUBNETWORK
  } else {
    $database->database_query("UPDATE se_subnets SET subnet_name='$subnet_name', subnet_field1_qual='$subnet_field1_qual', subnet_field1_value='$subnet_field1_value', subnet_field2_qual='$subnet_field2_qual', subnet_field2_value='$subnet_field2_value' WHERE subnet_id='$subnet[subnet_id]'");
    header("Location: admin_subnetworks.php?s=$s");
    exit();
  }
