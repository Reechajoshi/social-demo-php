...
{$chat_frame1}
<br><br>
{$chat_frame2} {$user->user_info.user_username}{$chat_frame3}
<br><br>
...
<tr>
<td class='chat_header'>{$chat_frame4}</td>
<td class='chat_header2' id='chat_onlineusers_header'{if $setting.setting_chat_showphotos == 1} style='text-align: center;'{/if}>&nbsp;</td>
</tr>
<tr>
...
<td rowspan='2' class='chat_onlineusersarea'>
  <div style='overflow-y: auto; height: 477px;' class='chat_onlineusers' {if $setting.setting_chat_showphotos == 1} style='text-align: center;'{/if}>
    <div class='chat_onlineusers_me'{if $setting.setting_chat_showphotos == 1} style='text-align: center;'{/if}><a href='{$url->url_create('profile', $user->user_info.user_username)}' target='_blank'>{if $setting.setting_chat_showphotos == 1}<img src='{$user->user_photo("./images/nophoto.gif")}' width='70' class='photo' border='0'><br>{/if}{$user->user_info.user_username|truncate:15:'...':true}</a></div>
...
<td class='chat_inputarea' style='height: 120px;'>
  ...
  <div class='buttons'>
    <div class='button' style='width: 25px;'><div class='button1' id='button_bold' onClick="button_toggle('button_bold');"><img src='./images/icons/chat_bold.png' border='0' class='icon' onselect="return false" alt='{$chat_frame5}'></div></div>
    <div class='button' style='width: 25px;'><div class='button1' id='button_italic' onClick="button_toggle('button_italic');"><img src='./images/icons/chat_italic.png' border='0' class='icon' alt='{$chat_frame6}'></div></div>
    <div class='button' style='width: 25px;'><div class='button1' id='button_underline' onClick="button_toggle('button_underline');"><img src='./images/icons/chat_underline.png' border='0' class='icon' alt='{$chat_frame7}'></div></div>
    <div class='button' style='width: 25px;'><div class='button1' id='button_smilie' onClick="button_picksmilie()"><img src='./images/icons/chat_smile.png' border='0' class='icon' alt='{$chat_frame8}'></div>
    ...
    <div class='button' style='width: 45px;'><div class='button_color1' id='button_color'><img src='./images/trans.gif' style='height: 16px; width: 100%;' border='0' onClick="button_pickcolor()" alt='{$chat_frame9}'>
  ...
  <img src='./images/icons/chat_clock1.gif' id='chat_timestamp_icon' border='0' class='icon' onClick='button_timestamp_toggle()' style='margin-top: 2px; margin-left: 2px;' alt='{$chat_frame10}'>
  <img src='./images/icons/chat_audio1.gif' id='chat_audio_icon' border='0' class='icon' onClick='button_audio_toggle()' style='margin-top: 2px; margin-left: 4px;' alt='{$chat_frame11}'>
...
<iframe src='chat_frame.php?task=blank&nocache={0|rand:10000}' id='chat_send' name='chat_send' style='display: none;'></iframe>
...
var time_diff = {/literal}{$server_time}{literal}-javascript_time.getTime();
...
function chat_addmsg() {
  ...
  // SEND MESSAGE
    ...
    document.getElementById('chat_content').innerHTML = document.getElementById('chat_content').innerHTML + '<div class="chat_message" id="message' + message_id + '"><span class="chat_timestamp" style="' + message_style + '">(' + timestamp + ') </span>{/literal}<b>{$user->user_info.user_username}</b>: {literal}' + chatMsg_withsmilies + '</div>';
...
function chat_update() {
  ...
  setTimeout("chat_update_go()", {/literal}{$setting.setting_chat_update*1000}{literal});
...
function ajaxFunction(task) {
  ...
  if(task == "chat_update") {
	...
	if(result == "success") {
	  ...
	  for(var counter=0;counter<onlineusers.length;counter++) {
	    ...
	    output_users += "<div class='chat_onlineuser' {/literal}{if $setting.setting_chat_showphotos == 1}style='text-align: center;'{/if}{literal}><a href='profile.php?user="+username+"' target='_blank'>";
	    {/literal}{if $setting.setting_chat_showphotos == 1}{literal}
	  ...
	  if(onlineusers_num == "1") {
	    document.getElementById('chat_onlineusers_header').innerHTML = onlineusers_num + " {/literal}{$chat_frame12}{literal}";
	  } else {
	    document.getElementById('chat_onlineusers_header').innerHTML = onlineusers_num + " {/literal}{$chat_frame13}{literal}";
	  }
	...
	// USER WAS PROBABLY KICKED
	  ...
	  document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame14}{literal}";
	...
	// UPDATE FAILED FOR SOME REASON, SO TRY AGAIN 5 TIMES BEFORE SHOWING CURTAIN
	  ...
	  if(update_attempts_remaining == 0) {
	    document.getElementById('chat_curtain').style.display = "block";
	    document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame15}{literal}";
	  }
  ...
  if(task == "chat_login") {
	...
	// LOGIN FAILED BECAUSE USER HAS BEEN KICKED
	  ...
	  document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame16}{literal}";

	// LOGIN FAILED BECAUSE USER HAS BEEN BANNED
	} else if(xmlHttp.responseText == "banned") {

	  document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame17}{literal}";

	// LOGIN FAILED BECAUSE CHAT HAS BEEN DISABLED BY THE ADMIN
	} else if(xmlHttp.responseText == "chatoff") {

	  document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame18}{literal}";

	// LOGIN FAILED FOR SOME REASON
	} else {

	  document.getElementById('chat_curtain').innerHTML = "{/literal}{$chat_frame19}{literal}";
